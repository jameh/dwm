...
exit_on_fail() {
	...
	if [ -n "$3" ]; then
		...
		_old_ifs="$IFS"
		IFS=$'\t'
		_pos_args=$(echo "$*" | cut -d "$IFS" -f 4-)
		IFS="$_old_ifs"
		"$_error_handler" "$_exit_code" $_pos_args
		[ "$?" -eq 0 ] && return 0
	fi
...
check_git() {
	...
	if [ "$?" -ne 0 ]; then
		exit_on_fail "Unstaged changes exist." 1
	fi
}
...
checkout_handler() {
	if [ "$1" -eq 128 ]; then
		...
		if [ "$_delete" = "y" ] && [ "$_delete" = "Y" ]; then
			...
			log "Deleting $_branch_name"
			git branch -D "$_branch_name" 1>&2 2> /dev/null
			log "Creating new branch $_branch_name"
			git checkout -b "$_branch_name" 1>&2 2> /dev/null
			return 0
		fi
	fi
}
